...
public interface ProdutoSwagger {
    ...
    @Operation(summary = "Fetch products filter", description = "Fetch some products based on an filter, all of them are optional.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Fetched all products successfully", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = ProdutoResponse.class))))
    })
    ResponseEntity<List<ProdutoResponse>> getBy(ProdutoFiltroRequest produtoFiltroRequest);

    @Operation(summary = "Fetch one product", description = "Fetch just one product by its id.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Register a product", description = "Register a product sending an body")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update a product", description = "Update a product sending some itens of the body or it all.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Deleted one product", description = "Deleted just one product by its id.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Fetch all products with discount", description = "Fetch all products and their data that just have discounts.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Fetched all products successfully", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = ProdutoResponse.class))))
    })
    ResponseEntity<List<ProdutoResponse>> getAllWithDiscount();

    @Operation(summary = "Removed discount", description = "Remove the discount of a product its id.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Rate a product", description = "Rate a product from 1 to 5 stars.")
    @ApiResponses(value = {
